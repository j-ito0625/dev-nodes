...
class JobShopGASchedulerV2ContinuousOnly:
    def __init__(self, dataset_path: str):
        """
        V2版連続作業制限のみ対応のジョブショップスケジューラ（デバッグ用）
        ...
        for constraint in self.order_constraints:
            ...
            if pred_id in schedule and succ_id in schedule:
                ...
                if time_diff < constraint['time_diff_min']:
                    penalty += (constraint['time_diff_min'] - time_diff) * 10  # V2の重み
            ...
            for i in range(len(equipment_tasks) - 1):
                ...
                for constraint in constraints:
                    if constraint['prev_pattern'] == curr_pattern and constraint['next_pattern'] == next_pattern:
                        penalty += 100  # V2の大きなペナルティ
        
        return penalty
    
    def crossover(self, parent1, parent2):
        """順序交叉（OX: Order Crossover）- V2版"""
    ...
    def run(self):
        """遺伝的アルゴリズムの実行（V2版）"""
        print("\nStarting GA V2 (Continuous Constraints Only Mode)...")
        print(f"Population size: {self.population_size}")
        print(f"Generations: {self.num_generations}")
        print(f"Crossover rate: {self.crossover_rate}")
        print(f"Mutation rate: {self.mutation_rate}")
        print(f"Continuous penalty weight: 100")
    ...
    def print_validation_report(self, chromosome):
        ...
        print("\n" + "="*60)
        print("SCHEDULE VALIDATION REPORT (V2 - Continuous Constraints Only)")
        print("="*60)
        
        print(f"\n✓ Makespan: {results['makespan']:.2f} minutes")
        print(f"  (Calculated as: {results['makespan']:.2f} = max(end_times) - 0)")
        print(f"✓ Schedule is valid: {results['is_valid']}")
        print("✓ Fixed tasks: DISABLED (debug mode)")
...
if __name__ == "__main__":
    ...
    scheduler = JobShopGASchedulerV2ContinuousOnly(dataset_path)
