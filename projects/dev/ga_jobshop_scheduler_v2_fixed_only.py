...
class JobShopGASchedulerV2FixedOnly:
    def __init__(self, dataset_path: str):
        """
        V2版固定タスクのみ対応のジョブショップスケジューラ（デバッグ用）
    ...
    def process_data(self):
        ...
        print(f"=== V2 Fixed Tasks Only Mode ===")
    ...
    def calculate_constraint_penalty(self, schedule):
        """制約違反のペナルティ計算（V2版、連続作業制限なし）"""
        penalty = 0
        
        # 順序制約のペナルティのみ（V2標準の重み）
        for constraint in self.order_constraints:
            pred_id = constraint['predecessor']
    ...
    def run(self):
        """遺伝的アルゴリズムの実行（V2版）"""
        print("\nStarting GA V2 (Fixed Tasks Only Mode)...")
    ...
    def print_validation_report(self, chromosome):
        ...
        print("\n" + "="*60)
        print("SCHEDULE VALIDATION REPORT (V2 - Fixed Tasks Only)")
        print("="*60)
        ...
        print("✓ Continuous constraints: DISABLED (debug mode)")
...
if __name__ == "__main__":
    ...
    scheduler = JobShopGASchedulerV2FixedOnly(dataset_path)
