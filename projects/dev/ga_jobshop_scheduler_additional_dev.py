...
class JobShopGASchedulerAdditional:
    ...
    def process_data(self):
        ...
        if not self.continuous_constraints_df.empty:
            for _, row in self.continuous_constraints_df.iterrows():
                eq = row['設備']
                if eq not in self.continuous_constraints:
                    self.continuous_constraints[eq] = []
                self.continuous_constraints[eq].append({
        ...
        for _, row in self.allocation_df.iterrows():
            pat = row['割り当て可能パターン']
            if pat not in self.allocation_patterns:
                self.allocation_patterns[pat] = []
            self.allocation_patterns[pat].append({
                'worker': row['作業者'],
                'equipment': row['設備']
            })
    ...
    def topological_sort_with_randomness(self):
        ...
        for c in self.order_constraints:
            pred, succ, lag = c['predecessor'], c['successor'], c['time_diff_min']
            if lag >= 0:  # 非負ラグのみ辺を張る
        ...
        result = []
        available = [t for t in self.regular_task_ids if in_degree[t] == 0]
        while available:
            random.shuffle(available)
            t = available.pop(0)
            result.append(t)
            for s in graph.get(t, []):
                in_degree[s] -= 1
                if in_degree[s] == 0:
                    available.append(s)

        for t in self.regular_task_ids:
            if t not in result:
                result.append(t)
        return result
    ...
    # (7) 二資源の交差空きスロット探索
    # ========================================================
    def invert_busy(self, busy_periods, horizon=10**9):
        if not busy_periods:
            return [(0, horizon)]
        busy_sorted = sorted(busy_periods, key=lambda x: x[0])
        free = []
        prev_end = 0
        ...
        if prev_end < horizon:
            free.append((prev_end, horizon))
        return free

    def find_available_slot(self, worker_busy, equipment_busy, duration, worker, equipment):
        worker_free = self.invert_busy(worker_busy.get(worker, []))
        equipment_free = self.invert_busy(equipment_busy.get(equipment, []))
        for ws, we in worker_free:
            for es, ee in equipment_free:
                start = max(ws, es)
                end = min(we, ee)
                if end - start >= duration:
                    return start
        return max(worker_free[-1][1], equipment_free[-1][1])

    # ========================================================
    # (5)(6) デコーダ修正: 先行固定タスク・負時刻対応
    # ========================================================
    def decode_chromosome(self, chromosome):
        schedule = dict(self.fixed_tasks)
        worker_busy, eq_busy = {}, {}

        for tid, info in self.fixed_tasks.items():
            w, e = info['worker'], info['equipment']
            worker_busy.setdefault(w, []).append((info['start_time'], info['end_time']))
            eq_busy.setdefault(e, []).append((info['start_time'], info['end_time']))

        min_lag = min((c['time_diff_min'] for c in self.order_constraints), default=0)
        earliest_time = min_lag if min_lag < 0 else 0

        for gene in chromosome:
            tid, w, e = gene['task'], gene['worker'], gene['equipment']
            dur, pat = self.tasks[tid]['duration'], self.tasks[tid]['pattern']

            start_time = self.find_available_slot(worker_busy, eq_busy, dur, w, e)
            start_time = max(start_time, earliest_time)

            # 後続=現在タスク
            for c in self.order_constraints:
                if c['successor'] == tid and c['predecessor'] in schedule:
                    pred_id = c['predecessor']
                    ref_time = schedule[pred_id]['end_time'] if c['pred_origin'] == '終了' else schedule[pred_id]['start_time']
                    if c['succ_origin'] == '開始':
                        start_time = max(start_time, ref_time + c['time_diff_min'])
                    else:
                        start_time = max(start_time, ref_time + c['time_diff_min'] - dur)

            # 先行=現在タスクかつ後続=固定タスク
            for c in self.order_constraints:
                if c['predecessor'] == tid and c['successor'] in self.fixed_tasks:
                    succ_id = c['successor']
                    succ_time = self.fixed_tasks[succ_id]['start_time'] if c['succ_origin'] == '開始' else self.fixed_tasks[succ_id]['end_time']
                    latest_finish = succ_time - c['time_diff_min']
                    latest_start = latest_finish - dur
                    if start_time > latest_start:
                        start_time = latest_start

            end_time = start_time + dur
            schedule[tid] = {'start_time': start_time, 'end_time': end_time, 'worker': w, 'equipment': e}

            worker_busy.setdefault(w, []).append((start_time, end_time))
            eq_busy.setdefault(e, []).append((start_time, end_time))

        return schedule

    def calculate_fitness(self, chromosome):
        """適応度（メイクスパン）の計算（新しい計算方法）"""
            ...
            for task_id, info in schedule.items():
                if info['equipment'] == equipment:
                    if task_id in self.fixed_tasks:
                        ...
                        pattern = self.tasks[task_id]['pattern']
                    equipment_tasks.append((info['start_time'], info['end_time'], pattern))
            
            equipment_tasks.sort()
            
            # 連続するタスクの制約をチェック
            for i in range(len(equipment_tasks) - 1):
                curr_pattern = equipment_tasks[i][2]
                next_pattern = equipment_tasks[i+1][2]
            ...
            for task_id, info in schedule.items():
                if info['equipment'] == equipment:
                    if task_id in self.fixed_tasks:
                        ...
                        pattern = self.tasks[task_id]['pattern']
                    equipment_tasks.append((info['start_time'], task_id, pattern))
                ...
                for constraint in constraints:
                    if constraint['prev_pattern'] == curr_pattern and constraint['next_pattern'] == next_pattern:
                        ...
                        validation_results['continuous_violations'].append({
                            'equipment': equipment,
                            'task1': equipment_tasks[i][1],
                            'task2': equipment_tasks[i+1][1],
    ...
    def print_validation_report(self, chromosome):
        ...
        print("\n" + "="*60)
        print("SCHEDULE VALIDATION REPORT (with Additional Constraints)")
        print("="*60)
        ...
        if not results['is_valid']:
            ...
            if results['continuous_violations']:
                ...
                for v in results['continuous_violations']:
                    ...
                    print(f"    Continuous work not allowed!")
        else:
    ...
    def run(self):
        """遺伝的アルゴリズムの実行"""
        print("Starting Genetic Algorithm with Additional Constraints...")
    ...
    def plot_convergence(self, best_fitness_history, avg_fitness_history):
        ...
        ax.set_title('GA Convergence with Additional Constraints')
        ax.legend()
