...
class JobShopGASchedulerAdditional:
    def __init__(self, dataset_path: str):
        """
        追加制約対応の遺伝的アルゴリズムを用いたジョブショップスケジューラ
    ...
    def decode_chromosome(self, chromosome):
        ...
        # 通常のリソース利用可能時刻（固定タスクがある場合は最早開始時刻から）
        earliest_time = min(task['start_time'] for task in self.fixed_tasks.values()) if self.fixed_tasks else 0
        worker_availability = {worker: earliest_time for worker in self.workers if worker != 'dummy-type-0001'}
        equipment_availability = {equipment: earliest_time for equipment in self.equipments if equipment != 'dummy-type-0002'}
...
if __name__ == "__main__":
    ...
    # スケジューラの初期化と実行
    scheduler = JobShopGASchedulerAdditional('toy_dataset_additional.xlsx')
